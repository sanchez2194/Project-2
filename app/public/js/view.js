...
$("#search-btn").on("click", function() {
  ...
  $.get("/api/" + searchedCharacter, function(data) {
    ...
    if (!data) {
      $("#well-section").append("<h2> Sorry, Singer not found :(. </h2>");
    }
    else {
      // otherwise
      ...
      $("#well-section").append("<h3>Singing Skills: " + data.singing_skills + "</h3>");
    }
  });
});
...
$("#search-youtube").on("click", function() {
  // save the character they typed into the character-search input
  var searchedVideo = $("#search-youtube")
    .val()
    .trim();

  // Using a RegEx Pattern to remove spaces from searchedCharacter
  // You can read more about RegEx Patterns later https://www.regexbuddy.com/regex.html
  searchedVideo = searchedVideo.replace(/\s+/g, "").toLowerCase();

  // run an AJAX GET-request for our servers api,
  // including the user's character in the url
  $.get("/api/" + searchedVideo, function(data) {
    // log the data to our console
    console.log(data);
    // empty to well-section before adding new content
    $("#video-details").empty();
    // if the data is not there, then return an error message
    if (!data) {
      $("#video-details").append("<h2> The force is not strong with this one. Your character was not found. </h2>");
    }
    else {
      // otherwise
      // append the character name
      <ul class="video-results glide__slides"></ul>
    }
  });
});
